...
class ConnectionServiceClient(metaclass=ConnectionServiceClientMeta):
    ...
    def cluster_path(project: str,region: str,cluster: str,) -> str:
        ...
        return "projects/{project}/regions/{region}/clusters/{cluster}".format(project=project, region=region, cluster=cluster, )

    @staticmethod
    def parse_cluster_path(path: str) -> Dict[str,str]:
        """Parses a cluster path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/regions/(?P<region>.+?)/clusters/(?P<cluster>.+?)$", path)
